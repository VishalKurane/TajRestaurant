...
async def main(req: func.HttpRequest) -> func.HttpResponse:
    logging.info("Processing a user login request.")
    ...
    try:
        ...
        cursor.execute('SELECT password_hash FROM UserTable WHERE email = ?', (email,))
        row = cursor.fetchone()
        ...
        if row:
            ...
            if stored_password_hash == password_hash:
                ...
                logging.warning(f"User {email} provided an incorrect password.")
                response_data = {"error": "Incorrect password"}
                response_status = 401
        else:
            # User does not exist
            logging.warning(f"User {email} not found in the database.")
