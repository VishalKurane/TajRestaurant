...
AZ_FUNCTION_USER_LOGIN = os.getenv('AZ_FUNCTION_USER_LOGIN')
...
def login():
    if request.method == 'POST':
        ...
        try:
            response = requests.post(AZ_FUNCTION_USER_LOGIN, json=login_data)
            
            # Log status code and response content for debugging
            logging.info(f"Status Code for AZ_FUNCTION_USER_LOGIN: {response.status_code}")
            # logging.info(f"Response Message for AZ_FUNCTION_USER_LOGIN: {response.text}")
